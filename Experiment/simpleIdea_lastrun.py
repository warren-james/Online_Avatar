...
for thisDecisionTrialLoop in DecisionTrialLoop:
    ...
    # keep track of which components have finished
    DrivingAvatarComponents = [DrivingKeys, text, text_2, DrivingBoundary, AvatarPlaced, AvatarDrive, LeftBuild, RightBuild]
    ...
    while continueRoutine:
        ...
        if LeftBuild.status == STARTED:  # only update if drawing
            ...
            LeftBuild.setFillColor(box_build, log=False)
            LeftBuild.setLineColor(box_build, log=False)
        ...
        if RightBuild.status == STARTED:  # only update if drawing
            ...
            RightBuild.setFillColor(box_build, log=False)
            RightBuild.setLineColor(box_build, log=False)
